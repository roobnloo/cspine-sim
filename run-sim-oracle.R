# Usage: Rscript run-sim-oracle.R --p=25 --q=50 --nobs=200 [--nrep=100] [--start_id=1]
...
setting_str <- sprintf("p%dq%d-n%d-d0.00", p, q, nobs)
metrics <- c(
...
message(sprintf("Settings: p=%d, q=%d, nobs=%d, nrep=%d, start_id=%d (oracle RegGMM, delta=0)", p, q, nobs, nrep, start_id))
