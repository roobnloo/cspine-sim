...
all_data <- readRDS(sprintf("data/p%dq%d-n%d-%s-varying-q.rds", p, q, n, model))

nrep <- length(all_data)
metrics <- c(
  "tpr", "fpr", "tpr_pop", "fpr_pop", "tpr_cov", "fpr_cov",
  "beta_err", "omega_err", "gamma_err", "mean_err", "omega_tpr", "omega_fpr", "q"
)
...
outpath <- file.path("out", sprintf("p%dq%d-n%d-%s-varying-q-result", p, q, n, model))
